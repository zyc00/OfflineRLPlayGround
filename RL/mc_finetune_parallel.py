"""PPO finetuning with MC-estimated Q-V advantage — parallel re-rollout.

Same algorithm as mc_finetune.py but uses independent large-scale envs
(num_mc_envs=512) to parallelize MC re-rollouts. For each timestep t,
all num_envs states are expanded into num_mc_envs and rolled out in one
parallel batch, drastically reducing wall-clock time.
...
Usage:
  ...
  python -m RL.mc_finetune_parallel --mc_samples 5 --num_envs 50 --num_steps 100 \
...
if __name__ == "__main__":
    ...
    if args.exp_name is None:
        args.exp_name = f"mc{args.mc_samples}_qv_parallel"
    run_name = f"{args.exp_name}__seed{args.seed}__{int(time.time())}"

    print(f"=== PPO Finetuning (MC{args.mc_samples} Q-V, parallel re-rollout) ===")
