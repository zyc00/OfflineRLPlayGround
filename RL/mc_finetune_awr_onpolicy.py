"""AWR finetuning with on-policy MC-estimated Q-V advantage — parallel re-rollout.

Same as mc_finetune_awr_parallel.py but uses the current policy (agent)
instead of an optimal policy for MC re-rollouts.
...
This is standard on-policy advantage estimation (as in PPO with MC returns).
No critic is trained. Actor-only AWR.

Usage:
  # MC5 AWR on-policy parallel (default)
  python -m RL.mc_finetune_awr_onpolicy --mc_samples 5

  # Tuning temperature
  python -m RL.mc_finetune_awr_onpolicy --mc_samples 5 --awr_beta 0.5
"""
...
if __name__ == "__main__":
    ...
    if args.exp_name is None:
        args.exp_name = f"mc{args.mc_samples}_awr_onpolicy"
    run_name = f"{args.exp_name}__seed{args.seed}__{int(time.time())}"

    print(f"=== AWR Finetuning (On-policy MC{args.mc_samples}, parallel re-rollout) ===")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            for t in tqdm(range(args.num_steps), desc=f"  MC re-rollout iter {iteration}", leave=False):
                ...
                #      mc_samples <= j < 2*mc_samples: V replica → sample from π (agent)
                ...
                # V replicas: sample action from current policy (on-policy)
                ...
                # 5. Follow current policy until all done (on-policy)
        ...
        # Precompute AWR weights (global normalization, outside minibatch loop)
        if args.norm_adv:
