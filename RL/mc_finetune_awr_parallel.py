"""AWR finetuning with MC-estimated Q-V advantage — parallel re-rollout.

Same as mc_finetune_parallel.py but replaces PPO update with AWR
(Advantage Weighted Regression). AWR uses exp(A/beta) weighted
supervised learning instead of PPO's clipped importance ratio.
...
Usage:
  # MC5 AWR parallel (default)
  python -m RL.mc_finetune_awr_parallel --mc_samples 5

  # Tuning temperature
  python -m RL.mc_finetune_awr_parallel --mc_samples 5 --awr_beta 0.5
"""
...
if __name__ == "__main__":
    ...
    if args.exp_name is None:
        args.exp_name = f"mc{args.mc_samples}_awr"
    run_name = f"{args.exp_name}__seed{args.seed}__{int(time.time())}"

    print(f"=== AWR Finetuning (MC{args.mc_samples} Q-V, parallel re-rollout) ===")
