"""CPU evaluation for Gaussian policy (accurate, matches pretrain eval)."""
import torch
import numpy as np
...
def evaluate_gaussian_cpu(
    ...
    """Evaluate Gaussian policy using CPU envs (deterministic: use mean action).

    Supports action chunking: predict horizon_steps actions, execute act_steps.
    """
    model.eval()
    ...
    while eps_done < n_episodes:
        ...
        # Init obs buffer: repeat first obs for conditioning history
        ...
        while step < max_episode_steps and not done:
            ...
            if cond_steps == 1:
                cond_obs = obs_buffer[-1]  # (num_envs, obs_dim)
            else:
                cond_obs = torch.stack(list(obs_buffer), dim=1)  # (num_envs, cond_steps, obs_dim)
            ...
            if no_action_norm:
                ...
                actions_chunk = (mean + 1.0) / 2.0 * (a_hi - a_lo) + a_lo
            ...
            for t in range(n_exec):
                if horizon_steps > 1:
                    action = actions_chunk[:, t]  # (num_envs, action_dim)
                else:
                    action = actions_chunk  # (num_envs, action_dim)
