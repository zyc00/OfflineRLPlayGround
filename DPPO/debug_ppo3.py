"""
Diagnostic 3: Test norm_adv=False to fix the collapse.
"""
import numpy as np
import torch
...
def do_ppo_update(model, obs_trajs, chains_trajs, rewards, dones, values,
                  n_epochs=1, lr=1e-5, use_reward_scaler=True):
    ...
    n_pos = (adv_flat > 0.01).sum().item()
    n_neg = (adv_flat < -0.01).sum().item()
    n_zero = len(adv_flat) - n_pos - n_neg
    print(f"    advantages: mean={adv_flat.mean():.4f} std={adv_flat.std():.4f} "
...
# ===== Test A: norm_adv=True (current, broken) =====
print("=== Test A: norm_adv=True, reward_scaler=True (original settings) ===")
model_a = build_model(norm_adv=True)
n_succ, obs_trajs, chains_trajs, rewards, dones, values = do_rollout(model_a, seed=42)
print(f"  pre-update: succ={n_succ}")
do_ppo_update(model_a, obs_trajs, chains_trajs, rewards, dones, values,
              n_epochs=1, use_reward_scaler=True)
n_succ, *_ = do_rollout(model_a, seed=0)
print(f"  post-update: succ={n_succ}")

# ===== Test B: norm_adv=False, reward_scaler=True =====
print("\n=== Test B: norm_adv=False, reward_scaler=True ===")
model_b = build_model(norm_adv=False)
n_succ, obs_trajs, chains_trajs, rewards, dones, values = do_rollout(model_b, seed=42)
print(f"  pre-update: succ={n_succ}")
do_ppo_update(model_b, obs_trajs, chains_trajs, rewards, dones, values,
              n_epochs=1, use_reward_scaler=True)
n_succ, *_ = do_rollout(model_b, seed=0)
print(f"  post-update: succ={n_succ}")

# ===== Test C: norm_adv=False, reward_scaler=False =====
print("\n=== Test C: norm_adv=False, reward_scaler=False ===")
model_c = build_model(norm_adv=False)
n_succ, obs_trajs, chains_trajs, rewards, dones, values = do_rollout(model_c, seed=42)
print(f"  pre-update: succ={n_succ}")
do_ppo_update(model_c, obs_trajs, chains_trajs, rewards, dones, values,
              n_epochs=1, use_reward_scaler=False)
n_succ, *_ = do_rollout(model_c, seed=0)
print(f"  post-update: succ={n_succ}")

# ===== Test D: norm_adv=False, no reward scaler, 10 epochs =====
print("\n=== Test D: norm_adv=False, no reward_scaler, 10 epochs ===")
model_d = build_model(norm_adv=False)
n_succ, obs_trajs, chains_trajs, rewards, dones, values = do_rollout(model_d, seed=42)
print(f"  pre-update: succ={n_succ}")
do_ppo_update(model_d, obs_trajs, chains_trajs, rewards, dones, values,
              n_epochs=10, use_reward_scaler=False)
for seed in [0, 100, 200]:
    n_succ, *_ = do_rollout(model_d, seed=seed)
    print(f"  post-update seed={seed}: succ={n_succ}")
