"""
Diagnostic 5: Test clip_adv_min=0 with LR sweep, and compare with filter-top-k.
"""
import numpy as np
import torch
...
def do_rollout(model, seed=0):
    ...
    for step in range(n_steps):
        ...
        chains_trajs.append(samples.chains.clone())
        ...
        for a_idx in range(act_steps):
            ...
            action = samples.trajectories[:, act_idx] if act_idx < samples.trajectories.shape[1] else samples.trajectories[:, -1]
...
def do_ppo_update(model, obs_trajs, chains_trajs, rewards, dones, values,
                  n_epochs=1, lr=1e-5, clip_adv_min=None):
    """PPO update. V starts from 0, so advantages ≈ rewards (for unfair baseline)."""
    ...
    for i in range(0, N, 256):
        ...
        with torch.no_grad():
            batch_lp = model.get_logprobs(
                {"state": obs_stacked[step_inds // n_envs, step_inds % n_envs]},
                chains_stacked[step_inds // n_envs, step_inds % n_envs])
    ...
    if clip_adv_min is not None:
        b_advantages = torch.clamp(b_advantages, min=clip_adv_min)
...
# Sweep with clip_adv_min=0
print("=== LR sweep: norm_adv=False, clip_adv_min=0, 1 epoch ===")
print(f"{'lr':>10s}  {'pre':>4s}  {'post_s0':>7s}  {'post_s1':>7s}")
for lr in [0, 1e-7, 1e-6, 3e-6, 1e-5]:
    ...
    if lr > 0:
        do_ppo_update(model, obs_trajs, chains_trajs, rewards, dones, values,
                      n_epochs=1, lr=lr, clip_adv_min=0)
    n_p0, *_ = do_rollout(model, seed=0)
    n_p1, *_ = do_rollout(model, seed=100)
    print(f"  {lr:>10.0e}  {n_pre:>4d}  {n_p0:>7d}  {n_p1:>7d}")

# Sweep with 5 epochs
print(f"\n=== LR sweep: norm_adv=False, clip_adv_min=0, 5 epochs ===")
print(f"{'lr':>10s}  {'pre':>4s}  {'post_s0':>7s}  {'post_s1':>7s}")
for lr in [0, 1e-7, 3e-7, 1e-6, 3e-6]:
    model = build_model(norm_adv=False)
    n_pre, obs_trajs, chains_trajs, rewards, dones, values = do_rollout(model, seed=42)
    if lr > 0:
        do_ppo_update(model, obs_trajs, chains_trajs, rewards, dones, values,
                      n_epochs=5, lr=lr, clip_adv_min=0)
    n_p0, *_ = do_rollout(model, seed=0)
    n_p1, *_ = do_rollout(model, seed=100)
    print(f"  {lr:>10.0e}  {n_pre:>4d}  {n_p0:>7d}  {n_p1:>7d}")
