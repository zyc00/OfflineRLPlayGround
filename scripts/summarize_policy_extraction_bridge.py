...
LOG_GLOB = "logs/policy_extraction_bridge/peb_*_seed*.log"
OUT_DIR = "logs/policy_extraction_bridge"

EVAL_RE = re.compile(r"EVAL @ iter (\d+): gpu_sr=([0-9.]+)")
BEST_RE = re.compile(r"Finetuning complete\. Best sr_once=([0-9.]+)")
NAME_RE = re.compile(r"peb_(.+)_seed(\d+)\.log$")
...
def main():
    ...
    stage_rows = []
    for stage in sorted(grouped.keys(), key=lambda x: idx.get(x, 999)):
        g = grouped[stage]
    ...
    with open(report, "w", encoding="utf-8") as f:
        f.write("# Policy Extraction Bridge Ablation\n\n")
        for s in stage_rows:
            f.write(
