"""
MLP: zscore+PCA(obs) → PCA(action) for PickCube-v1.

Same pipeline as PegInsertion version:
1. Z-score normalize obs, then PCA
2. PCA on actions (112D → 20D)
...
def load_data(dataset):
    ...
    return np.array(all_obs, np.float32), np.array(all_act, np.float32), action_dim
...
def main():
    ...
    print("Loading data...")
    all_obs, all_act, action_dim = load_data(dataset)
    N = len(all_obs)
    ...
    env.close()
    print(f"\nPickCube zscore+PCA(obs)→MLP→PCA(act) policy: SR = {np.mean(results):.1%}")
