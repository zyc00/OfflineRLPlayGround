"""PegInsertion: C) raw→zscore+PCA(act) and E) zscore+PCA(obs)→zscore+PCA(act)."""
import os
import torch
...
COND_STEPS = 2; HORIZON_STEPS = 16; ACT_STEPS = 8; MAX_EP_STEPS = 200; N_ACT_PCA = 20
...
def main():
    ...
    env = gym.make('PegInsertionSide-v1', obs_mode="state", control_mode="pd_joint_delta_pos",
    ...
    env.close()
    print(f"\nPegInsertion: C={sr_c:.1%}, E={sr_e:.1%}")
